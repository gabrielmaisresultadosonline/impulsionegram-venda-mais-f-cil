...
import { getBarberPlans } from "@/lib/plans";
import { trackSiteEvent } from "@/lib/pixel.functions";

export const Route = createFileRoute("/barbea")({
  head: () => ({
    meta: [
      {
        title: "POPULAR Barbearia — Mais clientes, mais seguidores",
      },
      {
        name: "description",
        content:
          "Atraia mais clientes para a sua barbearia. Planos Marketing Completo e Trimestral 10K com seguidores filtrados por região e CEP.",
      },
      {
        property: "og:title",
        content: "POPULAR Barbearia — Mais clientes, mais seguidores",
      },
      {
        property: "og:description",
        content:
          "Cresça no Instagram e preencha sua agenda com clientes da sua região. Pagamento seguro via InfinitePay.",
      },
    ],
  }),
...
function BarberPage() {
  ...
  useEffect(() => {
    void trackSiteEvent({ data: { type: "pageview", source: "barbea" } });
  }, []);
  ...
  return (
    <main className="min-h-screen">
      <Hero
        ...
        headline={["Mais clientes,", "Mais seguidores", "na sua Barbearia!"]}
        highlight="Público filtrado por região e CEP no automático."
        description="Basta uma configuração e você faz tudo direto pelo celular. Atraia clientes da sua cidade, região e pelo perfil do seu concorrente — sem esforço manual."
      />
      ...
      <section className="px-4 py-20" id="planos">
        <div className="mx-auto max-w-6xl">
          <h2 className="text-3xl font-extrabold text-balance md:text-4xl">
            Planos <span className="text-gradient-brand">para barbearia</span>
          </h2>
          <p className="mt-3 max-w-2xl text-muted-foreground">
            Escolha o ideal para a sua barbearia. Entrega em até 6 horas após a confirmação do pagamento.
          </p>

          <div className="mt-12 flex flex-wrap justify-center gap-6">
            {getBarberPlans().map((plan) => (
              <PlanCard
                key={plan.id}
                plan={plan}
                selected={false}
      ...
      <section className="px-4 pb-24">
        <div className="glass-panel mx-auto max-w-3xl rounded-3xl p-8 text-center md:p-12">
          <h2 className="text-2xl font-extrabold text-balance md:text-3xl">
            Pronto para <span className="text-gradient-brand">lotar sua barbearia?</span>
          </h2>
