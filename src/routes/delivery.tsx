...
import { getDeliveryPlans } from "@/lib/plans";
...
export const Route = createFileRoute("/delivery")({
  head: () => ({
    meta: [
      {
        title: "POPULAR Delivery — Mais pedidos no seu delivery",
      },
      {
        name: "description",
        content:
          "Encha seu delivery de pedidos. Planos Marketing Completo e Trimestral 10K com seguidores filtrados por região e CEP para restaurantes, lanchonetes e food trucks.",
      },
      {
        property: "og:title",
        content: "POPULAR Delivery — Mais pedidos no seu delivery",
      },
      {
        property: "og:description",
        content:
          "Atraia clientes famintos da sua região no Instagram e aumente os pedidos do seu delivery. Pagamento seguro via InfinitePay.",
      },
    ],
  }),
...
function DeliveryPage() {
  ...
  useEffect(() => {
    void trackSiteEvent({ data: { type: "pageview", source: "delivery" } });
  }, []);
  ...
  return (
    <main className="min-h-screen">
      <Hero
        ...
        headline={["Mais pedidos,", "Mais seguidores", "no seu Delivery!"]}
        highlight="Público filtrado por região e CEP no automático."
        description="Basta uma configuração e você faz tudo direto pelo celular. Atraia clientes com fome que estão perto do seu delivery — por cidade, bairro, CEP e até pelo perfil do seu concorrente."
      />
      ...
      <section className="px-4 py-20" id="planos">
        <div className="mx-auto max-w-6xl">
          <h2 className="text-3xl font-extrabold text-balance md:text-4xl">
            Planos <span className="text-gradient-brand">para delivery</span>
          </h2>
          <p className="mt-3 max-w-2xl text-muted-foreground">
            Ideal para restaurantes, lanchonetes, pizzarias, hamburguerias e food trucks. Entrega em
            até 6 horas após a confirmação do pagamento.
          </p>

          <div className="mt-12 flex flex-wrap justify-center gap-6">
            {getDeliveryPlans().map((plan) => (
              <PlanCard
                key={plan.id}
                plan={plan}
                selected={false}
      ...
      <section className="px-4 pb-24">
        <div className="glass-panel mx-auto max-w-3xl rounded-3xl p-8 text-center md:p-12">
          <h2 className="text-2xl font-extrabold text-balance md:text-3xl">
            Pronto para <span className="text-gradient-brand">lotar seu delivery de pedidos?</span>
          </h2>
      ...
      <SignupDialog
        ...
        redirectTo="/painel?source=delivery"
        source="delivery"
      />
