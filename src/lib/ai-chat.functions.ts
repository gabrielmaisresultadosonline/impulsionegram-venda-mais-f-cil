  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return { text: `Erro técnico (OpenAI ${response.status}). Verifique seu token e saldo no admin. Detalhe: ${errorText.includes('insufficient_quota') ? 'Saldo insuficiente na OpenAI.' : errorText.substring(0, 50)}` };
      }
